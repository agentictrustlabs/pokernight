...
export function buildCard(persona: Persona, env: Env, url: URL, onPersonaHost = false): AgentCardV1 {
  return {
    ...
    // TWO SKILLS, listed separately on purpose: take a turn, say something. A table reads this list to
    // decide what it may ask of an agent, so an agent that only advises is never handed a seat. No
    // `*.record` and no `*.review`: a house persona keeps no hands and coaches nobody, and a table
    // records a finished hand only to an agent whose card says it keeps them.
    skills: [
