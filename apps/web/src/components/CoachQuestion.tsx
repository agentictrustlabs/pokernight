 ...
 *
 * The room asks their agent (`poker.coach`): do you consult a coach, and has your person been asked?
 * Their agent answers from their playbook and their own preferences record, in their vault — so "asked
 * once" is a fact about them, not about this browser: another device or another room reads the same
...
export function CoachQuestion({ session, config, game = 'poker', onSetUp }: { session: AppSession | null; config: AuthConfig | null; game?: 'poker' | 'canasta'; onSetUp?: () => void }) {
  ...
  useEffect(() => {
    ...
    let alive = true;
    // THE APP'S DEFAULTS FIRST — everybody who connects from here gets the room's skills and its default
    ...
    const defaults = home && config?.home.origin
      ? (async () => {
          ...
          try {
            ...
            if (b.ok && b.applied?.length) { setSettingUp(`Your agent now has the room's skills and a coach — ${b.applied.join(', ')} set up at your Home.`); setTimeout(() => setSettingUp(null), 9000); }
    ...
    defaults.then(() => api.coachStatus(session.token, game))
      .then((r) => {
        // NO NAME, NO COACH — and said so. The room addresses a person's agent by its public name and
        ...
        // A coach is the room's default now, so the sheet stays until one is hired — a person who really
    ...
    return (
      <div className="sheet-backdrop" role="presentation">
        <div className="sheet coach-question" role="dialog" aria-modal="true" aria-labelledby="coachq-title">
          ...
          <p>
            The room talks to your own agent by its name — that is how it asks for advice, records your hands, and
    ...
    <div className="sheet-backdrop" role="presentation">
      <div className="sheet coach-question" role="dialog" aria-modal="true" aria-labelledby="coachq-title">
        ...
        <p>
          ...
          coach is a service somebody runs: it reads the {game === 'canasta' ? 'rounds' : 'hands'} the room records
        ...
        {/* ONE TRIP, NOT A RECIPE. The Home's connect ceremony puts the room's skills on the agent, names
        ...
        {onSetUp ? (
          <p className="hint">
            {show.advertises
              ...
              : <>Your agent does not have the room’s skills on its card yet ({skills.map((sk, i) => <span key={sk}><code>{sk}</code>{i < skills.length - 1 ? ', ' : ''}</span>)}). Your Home adds them, hires {game === 'canasta' ? 'Carol’s canasta coach' : 'Bob’s hold’em coach'} under a study grant you approve, and brings you straight back — one trip. Until then the house coach answers for you.</>}
          </p>
        ) : !show.advertises ? (
          <p className="hint">
            First, your agent needs the room’s skills on its card — at your Home, under{' '}
